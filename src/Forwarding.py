...
def forwardFrame(_interfaceName, _dstMacAddress, _payloadType, _payload):
    frame = scapy.Ether(dst=_dstMacAddress, type=_payloadType) / _payload
    answerList = scapy.srp(frame, iface=_interfaceName, timeout=5, verbose=False)[0]
...
interfaceMac = scapy.get_if_hwaddr(interfaceName)
